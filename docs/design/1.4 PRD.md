...
### FR-020: Live Component Showcase Page
**Description:** A `GET /showcase` route must provide a fully composed, theme-responsive page serving two simultaneous purposes: (a) a visual debugging surface where developers can inspect every nanoCSS component under any configured theme, and (b) a live sample the end-user can reference to verify their design choices before downloading.

The page must:
- Include **at least one instance of every nanoCSS component**, including Accordion, Badge, Banner, Breadcrumb, Button variants, Card variants, Carousel, Dropdown, Group, Hero, Loading, Modal, Nav, Pagination, Tabs, Tags, Theme Switcher, and Tooltip.
- Use **zero custom CSS** — every style rule must be satisfied by the compiled nanoCSS framework. If a component requires a layout wrapper, that wrapper must use nanoCSS utility classes only.
- Replace the global app navbar with its own **showcase-specific sticky glass navbar** (logo + icon on left; two links + a dropdown on right) so the page itself demonstrates a real nanoCSS nav in context.
- Be suppressed from rendering the global app navbar (via an `@suppress_app_nav` flag pattern in the layout).
- Carry **instructional, dogfooded copy** — explanatory text about nanoCSS embedded inside the component examples themselves, so both purposes (debug and user-facing sample) are served simultaneously.
- Include a live link back to the Configuration page.
- Be linked from the global app navbar on all other pages.

**Layout specification:**
- **Showcase navbar:** sticky, glass-morphism (`.{prefix}-sticky` + `.{prefix}-glass`), full-width. Logo (nano_icon + "nanoCSS") left; two nav links + one dropdown menu right.
- **Two-column body:** left pane = sticky accordion acting as in-page navigation (section title + description per entry); right pane = main content area.
- **Right-pane order:** Hero Banner → 3 Cards → explanatory text → Carousel → [remaining components in contextually logical groupings].

**Priority:** 🟧 Should Have (Sprint 12)

### FR-021: Sticky Utility Class
**Description:** The framework must expose a `.{prefix}-sticky` utility class. When applied to any element, it sets `position: sticky; top: 0` and promotes the element to the appropriate z-index layer (`$nanocss-z-sticky`). The class must be composable with `.{prefix}-glass` and must not conflict with `position: relative` parent constraints. A z-index variable `$nanocss-z-sticky` already exists in `_variables.scss`; this FR consummates it.
**Priority:** 🟧 Should Have (Sprint 12)

...
> _Nothing pending — all changes shipped in Sprint 12._

---

## [0.12.0] — 2026-05-10 — Sprint 12 · Live Component Showcase

> **189 specs · 0 failures · 0 RuboCop offences · 4 Brakeman warnings (pre-existing)**

### Added
- UC-059: `.nanocss-sticky` utility class in `_utilities.scss` (`position: sticky; top: 0; z-index: 1020`). Also added `.nanocss-glass` utility (`color-mix` tint + `backdrop-filter`) that auto-inverts in dark mode.
- UC-060: `.nanocss-accordion` first-class component — exclusive (shared `name` attribute, browser-native) and independent modes; CSS-only `▸` chevron rotates 90° on open; all values from theme tokens. Component Catalogue updated with wrapper pattern and both modes.
- UC-061: `GET /showcase` route → `ShowcaseController#show`; dedicated `layouts/showcase.html.erb` omitting `application.css`; app navbar wrapped in `unless @suppress_app_nav`; "Showcase" link added to global navbar. Showcase renders its own `.nanocss-nav.nanocss-sticky.nanocss-glass` sticky header.
- UC-062: Full two-column `/showcase` page — all 19 nanoCSS components demonstrated with instructional copy; sticky accordion sidebar; `min-width: 0` grid cells; responsive at 375px and 1280px. `spec/lint/showcase_custom_css_spec.rb` enforces no `style=` attributes.
- Sprint 11 (logged here): Retrofitted Phase 1 & 2 docs (UC-055), split sprint plans (UC-056), generated Agent Context files (UC-057), asserted YAML compliance (UC-058).

### Changed
- UC-059: Removed bespoke `position: sticky !important` from `application.css` `.nanocss-nav` block; app layout nav now uses the `.nanocss-sticky` utility class.
- UC-060: Removed `.preview-accordion-mb` / `.preview-accordion-text` from `application.css`.

### Fixed
- Carousel: inner classes are `.carousel-track`, `.carousel-slide`, `.carousel-btn.carousel-prev/next` — unprefixed, nested inside `.{prefix}-carousel`. Navigation uses native `onclick="carouselScroll()"`, not Stimulus.
- Badges: modifier pattern is `.nanocss-badge.success` / `.warning` / `.danger` — modifier on the base element, not BEM `-success` suffix.
- Modal: `<dialog class="{prefix}-modal">` + native `showModal()` / `close()`. A `<div>` never matched the SCSS selector `dialog.{prefix}-modal`.
- Tabs: pure CSS radio-input + label + `.panel` pattern. No Stimulus involved.
- Progress: native `<progress value="" max="">` styled by `_base.scss`. No wrapper class exists.
- Text Banner (alerts): the nanoCSS Banner component is `<hgroup>` — no class, globally styled. Showcase now demonstrates four semantic message examples using the real component.
- `.nanocss-glass` formula: was `calc(1 - surface-opacity)` (inverted); now `color-mix(in srgb, neutral-50 85%, transparent)` — dark-mode aware.
- 375px grid overflow: added `min-width: 0` to `.showcase-layout > *`; accordion min-content width was expanding the sidebar past its grid track.
- Showcase dark mode: removed hardcoded `body { background: #f3f4f6 }` from layout; `_base.scss` now owns body background via `var(--{prefix}-neutral-100)`, which inverts under dark mode tokens.
- Showcase chrome bleed: dedicated showcase layout omits `application.css` so components render against the default nanoCSS stylesheet, not the Obsidian dark chrome.

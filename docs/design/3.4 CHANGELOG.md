...
- UC-062 fix: Showcase rebuilt with correct markup contracts — all component classes verified directly against SCSS source and the live component catalogue before use. See "Post-Sprint Corrections" in `3.2 SPRINT-PLAN-12.md`.

### Fixed
- UC-062: Carousel markup corrected — inner classes are `.carousel-track`, `.carousel-slide`, `.carousel-btn.carousel-prev/next` (unprefixed, nested inside `.{prefix}-carousel`). Prev/Next use native `onclick="carouselScroll()"` not Stimulus.
- UC-062: Badge modifier pattern corrected — `.nanocss-badge.success`, `.nanocss-badge.warning`, `.nanocss-badge.danger` (modifier class on base element, not BEM `-success` variant).
- UC-062: Modal corrected — must be a `<dialog class="{prefix}-modal">` element, opened/closed via native `showModal()`/`close()` API. Previous implementation used a `<div>` with Stimulus; the SCSS selector `dialog.{prefix}-modal` never matched.
- UC-062: Tabs corrected — pure CSS radio-input pattern (`<input type="radio">` + `<label>` + `<div class="panel">`). Previous implementation used a Stimulus-based approach with invented class names that don't exist in `_tabs.scss`.
- UC-062: Progress corrected — the actual component is the native `<progress value="" max="">` element, styled by `_base.scss`. No `.nanocss-progress-bar` wrapper class exists.
- UC-062: Alerts section corrected — the nanoCSS "Banner" component is `<hgroup>` (styled globally, no class). No alert/notification banner component exists yet; section now demonstrates the Text Banner component with four semantic message examples.
- UC-062: `.nanocss-glass` formula corrected — was `calc(1 - surface-opacity)` which inverted the alpha (opaque for Obsidian, transparent for standard presets). Now uses `color-mix(in srgb, neutral-50 85%, transparent)` so the glass tint auto-inverts in dark mode.
- UC-062: 375px horizontal scroll fixed — grid cells lacked `min-width: 0`, causing the accordion's min-content width to expand the sidebar past its grid track. Adding `min-width: 0` to `.showcase-layout > *` contains it.
- UC-062: Showcase layout dark mode fixed — removed hardcoded `body { background: #f3f4f6; }` from showcase layout; `_base.scss` now controls body background via `var(--{prefix}-neutral-100)` which inverts correctly under dark mode tokens.
- fix: Showcase layout separated from application.css chrome — `ShowcaseController` uses `layout "showcase"` which omits the Obsidian dark chrome styles. Components render against the default nanoCSS stylesheet.

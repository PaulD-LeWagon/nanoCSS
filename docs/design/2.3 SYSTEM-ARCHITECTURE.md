...
### `GET /showcase` → `ShowcaseController#show`
Full-page live component showcase. Renders every nanoCSS component in a composed two-column layout with instructional dogfooded copy. Sets `@suppress_app_nav = true` so the global app navbar is suppressed; the page provides its own sticky glass navbar built from nanoCSS components. No custom CSS permitted on this page — all layout and styling is via nanoCSS utility and component classes. See FR-020, ADR-010.
...
│   ├── _mixins.scss
│   ├── _utilities.scss           # includes .{prefix}-sticky (ADR-009, UC-059)
│   ├── _components.scss          # @use index pointing to components/ partials (UC-032)
│   ├── components/               # one file per component (UC-032)
│   │   ├── _accordion.scss       # promoted from bare <details> — ADR-008, UC-060
│   │   ├── _badges.scss
│   │   ├── _banner.scss
│   │   ├── _breadcrumbs.scss
│   │   ├── _buttons.scss
...
│   └── nanocss.min.css           # Sass compressed output (UC-040, Sprint 9)
└── application.css               # Thin chrome shims only — bespoke rules reduced by Sprint 8/12
```

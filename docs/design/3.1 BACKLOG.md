...
## Sprint 11 — Studio Onboarding & Methodology Alignment · _Closed_

> **Sprint Goal:** Complete the project onboarding and methodology alignment.
> **Sprint Dates:** 2026-05-07
> **Delivered:** 8 pts

### UC-055 · Retrofit Phase 1 & 2 design documents · 3 pts · 🟢 Done
### UC-056 · Split monolithic SPRINT-PLAN.md · 2 pts · 🟢 Done
### UC-057 · Generate root Agent Context files · 1 pts · 🟢 Done
### UC-058 · Assert YAML frontmatter compliance · 2 pts · 🟢 Done
---

## Sprint 12 — Live Component Showcase · _Closed_

> **Sprint Goal:** Ship the `/showcase` page — a fully self-contained, zero-custom-CSS live component showcase that serves simultaneously as a visual debugging surface for developers and a live theme sample for end-users. Block deliverables: `.{prefix}-sticky` utility, `_accordion.scss` first-class component, `ShowcaseController`, and the full two-column page layout with all 19 components.
> **Sprint Dates:** 2026-05-10
> **Delivered:** 11 pts (100%) · Post-sprint QA corrections applied same day — see `3.2 SPRINT-PLAN-12.md`

---

### UC-059 · `.{prefix}-sticky` Utility Class · 🟧 Should Have · 1 pt · 🟢 Done

**As an** indie developer,
**I want** a `.{prefix}-sticky` utility class in my downloaded nanoCSS framework,
**so that** I can pin navbars and sidebars without writing any custom CSS.

**Acceptance Criteria:**
- AC1: `_utilities.scss` exports `.#{$prefix}-sticky { position: sticky; top: 0; z-index: $nanocss-z-sticky; }`.
- AC2: Composing `.{prefix}-sticky` with `.{prefix}-nav` and `.{prefix}-glass` produces a sticky glass navbar — no bespoke CSS required.
- AC3: `$nanocss-z-sticky: 1020 !default` variable already defined in `_variables.scss` — this UC only consummates it.
- AC4: Bespoke `position: sticky !important` rules removed from `application.css`.
- AC5: Unit spec asserts the compiled output contains `position: sticky` for the default prefix.

**Dependencies:** None
**ADR:** ADR-009
**Status:** 🟢 Done — Sprint 12 (2026-05-10). `.nanocss-sticky` added to `_utilities.scss`; bespoke `position: sticky` removed from `application.css`; app nav updated to use the class; `spec/services/sticky_utility_spec.rb` added (3 specs green).

---

### UC-060 · Accordion as First-Class nanoCSS Component · 🟧 Should Have · 2 pts · 🟢 Done

**As an** indie developer,
**I want** a `.{prefix}-accordion` wrapper component with exclusive and independent modes,
**so that** I can use styled, orchestrated collapsible panels without writing custom CSS or JS.

**Structure:** `.{prefix}-accordion` is placed on a **wrapper element** (`<div>` or `<section>`). Individual `<details>/<summary>` items live inside it as children.

**Exclusive mode (one open at a time):** all `<details>` share the same `name` attribute — native browser mutual exclusivity, zero JS (Chrome 120+, Firefox 130+, Safari 17.2+).

**Independent mode (any open at a time):** omit the `name` attribute (or use unique values) — each panel toggles independently. Signalled visually with a `.{prefix}-accordion--independent` modifier on the wrapper.

**Acceptance Criteria:**
- AC1: `app/assets/stylesheets/nanocss/components/_accordion.scss` created with `.#{$prefix}-accordion` on the wrapper element.
- AC2: Wrapper applies border, border-radius, and item dividers (`border-bottom` on each `<details>`) — all values from theme tokens, no hardcoded values.
- AC3: `<summary>` carries a CSS-only chevron (`::after` pseudo-element, content `"▸"`) that rotates 90° when the parent `<details>` is `[open]` — zero JS.
- AC4: `details[open] > summary` applies a subtle background tint using `--{prefix}-neutral-100`; sibling content area has standard `padding` from `--{prefix}-space-md`.
- AC5: `.{prefix}-accordion--independent` modifier exists and removes the visual mutual-exclusivity indicator (border-left accent) — functional difference comes from the HTML `name` attribute, not CSS.
- AC6: Component Catalogue (`/components#accordion`) copy snippet updated to the wrapper structure with a named-`<details>` example (exclusive) and a no-name example (independent).
- AC7: Bespoke `.preview-accordion-mb` / `.preview-accordion-text` rules removed from `application.css`.
- AC8: All existing specs pass; one new unit spec asserts the compiled CSS includes `.nanocss-accordion` and `.nanocss-accordion--independent`.

**Dependencies:** UC-059
**ADR:** ADR-008
**Status:** 🟢 Done — Sprint 12 (2026-05-10). `_accordion.scss` created; catalogue snippet updated to wrapper pattern with exclusive/independent examples; `.preview-accordion-mb/.preview-accordion-text` removed from `application.css`; `spec/services/accordion_component_spec.rb` added (3 specs green).

---

### UC-061 · Showcase Page — Scaffold, Route, Nav Suppression & App Navbar Link · 🟥 Must Have · 3 pts · 🟢 Done

**As an** indie developer,
**I want** a `/showcase` page accessible from the app navbar,
**so that** I can navigate directly to the live component demonstration.

**Acceptance Criteria:**
- AC1: `GET /showcase` route added to `config/routes.rb` → `ShowcaseController#show`.
- AC2: `ShowcaseController` sets `@suppress_app_nav = true` in `show` action.
- AC3: `application.html.erb` wraps the inline `<nav class="nanocss-nav">` block in `unless @suppress_app_nav`.
- AC4: The global app navbar gains a "Showcase" link pointing to `showcase_path` on all pages where it is rendered.
- AC5: The Showcase page renders its own sticky glass navbar (`.{prefix}-nav.{prefix}-sticky.{prefix}-glass`) with: nano_icon + "nanoCSS" wordmark on the left; "Catalogue" and "Configure →" links on the right; a dropdown menu (items: Catalogue, Configure, Download).
- AC6: System spec: `GET /showcase` returns 200; page does not contain the app navbar's `<ul class="nav-menu">` element; page does contain a `.nanocss-nav` element.
- AC7: System spec: the app navbar on `GET /` contains a link to `/showcase`.

**Dependencies:** UC-059, UC-060
**ADR:** ADR-010
**Status:** 🟢 Done — Sprint 12 (2026-05-10). `ShowcaseController` created; route added; layout nav wrapped in `unless @suppress_app_nav`; "Showcase" link added to app navbar; showcase view has its own `.nanocss-nav.nanocss-sticky.nanocss-glass` navbar; system specs green.

---

### UC-062 · Showcase Page — Full Component Layout · 🟥 Must Have · 5 pts · 🟢 Done

**As an** indie developer,
**I want** the `/showcase` page to render every nanoCSS component in a composed two-column layout with instructional copy,
**so that** I can visually verify my theme choices against real component examples before downloading.

**Acceptance Criteria:**
- AC1: Zero custom CSS — every style rule on the page is satisfied by the compiled nanoCSS framework. Verified by a grep spec: `spec/lint/showcase_custom_css_spec.rb` asserts no `style=` attributes in `app/views/showcase/`.
- AC2: Left pane — sticky accordion sidebar: `<aside class="{prefix}-sticky">` wrapping a single `<div class="{prefix}-accordion">` in **independent mode** (no `name` attrs — all sections can be browsed without collapsing others). Each `<details>` item maps to one right-pane section; `<summary>` is the section name; revealed body has a one-sentence description and an anchor link.
- AC3: Right pane — sections in this exact order: Hero (`#hero`) → Cards grid (`#cards`) → text copy (`#copy`) → Carousel (`#carousel`) → Interactive (`#interactive`: Tabs + Modal + standalone Dropdown) → Labels (`#labels`: Badges + Tags + Loading) → Navigation (`#navigation`: Breadcrumbs + Pagination + Nav) → Utilities (`#utilities`: Tooltip + Progress + Group) → Alerts (`#alerts`: Banner all four semantic variants) → Theming (`#theming`: Theme Switcher demo + Configure link).
- AC4: All 19 components appear at least once. The component list: Accordion, Badge, Banner, Breadcrumb, Button (variants), Card (variants), Carousel, Dropdown, Group, Hero, Loading, Modal, Nav, Pagination, Tabs, Tags, Theme Switcher, Tooltip, and the Sticky+Glass composed nav at the top.
- AC5: Instructional copy is dogfooded into the examples (e.g. the Hero headline reads "Build bespoke CSS frameworks in seconds"; the carousel slides explain nanoCSS features; card headers name real component use cases).
- AC6: The `#theming` section contains a visible link back to `/configure` using a `.{prefix}-btn` class.
- AC7: System spec at 375 px and 1280 px viewports: page renders without horizontal scroll; left accordion collapses below the mobile breakpoint; right pane is single-column on mobile.
- AC8: System spec: each right-pane `<section id="…">` can be directly linked from the left accordion anchor.

**Dependencies:** UC-059, UC-060, UC-061
**Status:** 🟢 Done — Sprint 12 (2026-05-10). Full two-column showcase view with all 19 components; no `style=` attributes; responsive at 375px and 1280px; `spec/lint/showcase_custom_css_spec.rb` + `spec/system/showcase_layout_spec.rb` added (10 specs green).
